...
- repo_path: Repository identifier (GitHub URL, Azure DevOps URL, or local path)
  Examples: 'https://github.com/user/repo', 'https://dev.azure.com/org/project/_git/repo', '/home/user/project'""",
    ...
    @mcp_server.tool(
        name="explain_code",
        description="""Ask natural language questions about a codebase and get comprehensive AI-powered explanations.

This tool uses an AI agent to explore repositories intelligently and provide detailed explanations of code structure, functionality, and design patterns.

PARAMETERS:
- repo_url: Repository identifier (GitHub URL, Azure DevOps URL, or local path)
  Examples: 'https://github.com/user/repo', 'https://dev.azure.com/org/project/_git/repo', '/home/user/project'
- question: Natural language question about the code
  Examples: 'How does authentication work?', 'What is the main application flow?', 'Where are the API endpoints defined?'
- context: Optional additional context as a dictionary

CAPABILITIES:
- Automatically clones repositories if needed
- Explores codebase structure intelligently
- Identifies critical files and components
- Reads relevant source code and documentation
- Generates comprehensive explanations with code examples
- Provides file references and line numbers
- Creates Mermaid diagrams for complex flows when helpful

RESPONSE FORMAT:
- question: The original question asked
- explanation: Comprehensive AI-generated explanation
- repository: Repository that was analyzed
- metadata: Information about AI provider, model, and tools used
- error: Error message if something went wrong""",
    ...
    async def explain_code(
        ...
        """
        Use AI to answer natural language questions about a codebase.

        This tool leverages an AI agent that can explore repositories, read source code,
        analyze documentation, and provide comprehensive explanations about how code works.

        Args:
            repo_url (str): URL or path to the repository to analyze
            question (str): Natural language question about the code
            context (dict, optional): Additional context to help answer the question
            ...
            dict: Response with format:
                {
                    ...
                    "repository": str,      # Repository analyzed
                    "metadata": {           # AI provider info and tools used
                        "provider": str,
                        "model": str,
                        "tools_used": List[str]
                    },
        ...
        Note:
            - The AI agent will automatically handle repository cloning if needed
            - Uses ReAct pattern to explore the codebase intelligently
            - Can read source code, documentation, and analyze structure
            - Provides detailed explanations with code examples and references
        """
        try:
            logger.info(f"Processing code explanation request for {repo_url}")
            logger.debug(f"Question: {question}")
            
            # Initialize the CodeExplanationAgent with this MCP server instance
            agent = CodeExplanationAgent(mcp_server)
            
            # Use the synchronous method since MCP tools are sync
            result = agent.explain_code_sync(repo_url, question, context)
            
            logger.info("Successfully processed code explanation request")
            return result
            
        except Exception as e:
            logger.error(f"Error in explain_code tool: {e}", exc_info=True)
            return {
                "question": question,
                "error": f"Failed to generate code explanation: {str(e)}",
                "repository": repo_url
            }
